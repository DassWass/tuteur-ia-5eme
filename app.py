...
# 1. Utilisation de st.cache_resource pour le modèle (Objet lourd)
@st.cache_resource(ttl=3600)
def make_model(system_prompt: str):
    # 2. Utilisation de la VRAIE API (plus de MockModel !)
...
def generate_next(chat_session, matiere: str, sujet: str, difficulty: str, is_first: bool) -> tuple:
    ...
    # 3. Boucle de Retry en cas de dépassement de quota
        ...
        try:
            ...
            if "429" in erreur_str or "quota" in erreur_str or "exhausted" in erreur_str:
                ...
                if switch_to_next_key():
                    continue # On retente avec la nouvelle clé
                else:
...
if not st.session_state.seance_lancee:
    ...
    # 4. Le bouton classique lance la séance sans bloquer les autres clics
